...
class OBJECT_OT_AddColors(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for cont_mat in range(10):
            ...
            material_value = bpy.data.particles["ParticleSettings"].dupli_weights['Ico_' + str(9-cont_mat) + ': 1']
...
class OBJECT_OT_ResetButton(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        nombreObjeto = "Sphere"  
        found = 'Sphere' in bpy.data.objects

        if(found): 
            bpy.data.objects[nombreObjeto].hide = False
            bpy.ops.object.particle_system_remove()
            bpy.data.particles.remove(bpy.data.particles['ParticleSettings'])
    ...
    def execute(self, context):
        ...
        for x in range(int(total_states)):
            ...
            try:
                #Sets the path where the files will be stored, by default the same as the datafile
                if dir_image_path == "":
                    dir_image_path = bpy.context.scene.iso_render.path
        ...
        shutil.make_archive(dir_image_path,'zip',dir_image_path)
...
def register():
    ...
    bpy.utils.register_class(PanelInfoShortcuts)
...
def unregister():
    ...
    bpy.utils.unregister_class(PanelInfoShortcuts)
